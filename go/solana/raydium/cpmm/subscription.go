...
// Swap resolves one transaction signature into a configured Raydium CPMM swap.
//
// Parameters:
...
func (s *SwapSubscriber) Swap(ctx context.Context, poolAddress onchainSolana.Address, signature onchainSolana.Signature) (*SwapEvent, error) {
	if s == nil || s.source == nil {
		return nil, fmt.Errorf("failed to resolve raydium cpmm swap: subscriber=null")
	}
	pool, ok := s.pools[poolAddress]
	if !ok {
		return nil, fmt.Errorf("failed to resolve raydium cpmm swap: pool=invalid")
	}
	if signature.IsZero() {
		return nil, fmt.Errorf("failed to resolve raydium cpmm swap: signature=empty")
	}
	if ctx == nil {
...
func resolveSwap(ctx context.Context, source transactionSource, pool Pool, signature onchainSolana.Signature) (*SwapEvent, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to resolve raydium cpmm swap: %w", err)
	}
	if transaction == nil {
		return nil, fmt.Errorf("failed to resolve raydium cpmm swap: transaction=null")
	}
