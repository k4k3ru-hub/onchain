...
func (c *Client) captureArrayWindow(ctx context.Context, poolAddress onchainSolana.Address, requests []ExactInputRequest) (QuoteBatch, error) {
	if c == nil || c.snapshots == nil {
		return QuoteBatch{}, fmt.Errorf("failed to capture raydium clmm array window: account_snapshot_provider=null")
	}
	if len(requests) == 0 {
		return QuoteBatch{}, fmt.Errorf("failed to capture raydium clmm array window: requests=empty")
	}
	configured, exists := c.pools[poolAddress]
	if !exists {
		return QuoteBatch{}, fmt.Errorf("failed to capture raydium clmm array window: pool=invalid pool_address=%q", poolAddress.String())
	}
	if ctx == nil {
	...
	for attempt := 0; attempt < 5; attempt++ {
		if err := ctx.Err(); err != nil {
			return QuoteBatch{}, fmt.Errorf("failed to capture raydium clmm array window: %w", err)
		}
		...
		if len(selected) > maximum {
			return QuoteBatch{}, fmt.Errorf("failed to capture raydium clmm array window: %w: max_array_count=%d", onchainSolana.ErrQuoteArrayLimit, maximum)
		}
...
func (s *StateCache) Warm(ctx context.Context) error {
	...
	s.mu.Unlock()
	local := &Client{initialArrayCount: s.client.initialArrayCount, maxArrayCount: s.client.maxArrayCount, accounts: s.client.accounts, programID: s.client.programID, pools: s.client.pools, configs: make(map[onchainSolana.Address]AMMConfig)}
...
func windowDirections(pool Pool) []ExactInputRequest {
	return []ExactInputRequest{{InputMint: pool.Token0Mint, AmountIn: 1}, {InputMint: pool.Token1Mint, AmountIn: 1}}
}
