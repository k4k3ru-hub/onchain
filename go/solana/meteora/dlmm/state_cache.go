...
func (s *StateCache) QuoteExactInputs(ctx context.Context, requests []ExactInputRequest) (CachedQuote, error) {
	...
	if generation != s.generation {
		return CachedQuote{}, fmt.Errorf("failed to quote cached meteora dlmm state: state changed during calculation")
	}
...
func (p *cacheSnapshotProvider) AccountSnapshot(ctx context.Context, requested []solana.Address) (*solana.AccountSnapshot, error) {
	...
	// Clock belongs to the snapshot but is not watched: otherwise every slot
	// invalidates the cache. Quotes retain the original snapshot timestamp and
	// Clock until the bounded maxAge expires; host time never advances chain time.
	p.addresses = nil
	for _, address := range addresses {
