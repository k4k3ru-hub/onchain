...
2026-09-23。`lp-protection-20260923-v3`。
...
- 各区間の末尾headerを取得前後で照合してからcheckpointを進める。空の正常なログ応答も成功。
...
- reorgで開始根拠／保存末尾が一致しなければ、保存証拠を破棄し、その評価では未判定を返す。次の評価で再構築できなければ未判定のまま。
