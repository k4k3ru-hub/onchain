...
func stateFixture(t *testing.T) (*StateCache, *stateReaderFake, QuotePairParams) {
	...
	f.obj = &sui.Object{Address: testAddress("0x9"), Version: 1, Move: &sui.MoveObject{Type: "0x1::pool::Pool<0x2::sui::SUI,0x3::usdc::USDC,0x1::fee::FEE>", JSON: json.RawMessage(`{"coin_a":"10000000","coin_b":"10000000","sqrt_price":"18446744073709551616","liquidity":"1000000","tick_current_index":{"bits":0},"fee":3000,"fee_protocol":100000,"unlocked":true,"tick_spacing":1,"tick_map":{"id":"0xb"}}`)}}
	c, err := NewStateCache(f, f.obj.Address, 30*time.Second)
	if err != nil {
		t.Fatal(err)
	}
	p := Pool{Address: f.obj.Address}
	params := QuotePairParams{Bid: QuoteExactInputParams{Pool: p, AmountIn: big.NewInt(1000), A2B: true, SqrtPriceLimit: sqrtAtTick(-443636)}, Ask: QuoteExactOutputParams{Pool: p, AmountOut: big.NewInt(1000), A2B: false, SqrtPriceLimit: sqrtAtTick(443636)}}
	return c, f, params
}
...
func TestCachedPairRoundingAndReuse(t *testing.T) {
	...
	if r.Bid.AmountIn.Uint64() != 1000 || r.Bid.AmountOut.Uint64() != 996 || r.Bid.FeeAmount != 3 || r.Ask.AmountIn.Uint64() != 1004 || r.Ask.AmountOut.Uint64() != 1000 || r.Ask.FeeAmount != 3 {
...
func TestCacheRejectsInvalidationAndUnavailableState(t *testing.T) {
	for _, kind := range []string{"floor", "during", "expired", "paused", "rpc", "cancel"} {
		t.Run(kind, func(t *testing.T) {
			...
			case "paused":
				f.obj.Move.JSON = json.RawMessage(strings.Replace(string(f.obj.Move.JSON), `"unlocked":true`, `"unlocked":false`, 1))
			case "rpc":
				f.fail = sentinel
			case "cancel":
...
func TestBitmapBoundariesAndCrossing(t *testing.T) {
	...
	r, err := c.quote(context.Background(), s, big.NewInt(20000), true, true, sqrtAtTick(-1000))
	if err != nil || r.AmountOut.Sign() == 0 {
		t.Fatalf("crossing: %+v %v", r, err)
	}
	f.missingTick = true
	s.nets = map[int32]*big.Int{}
	if _, err = c.quote(context.Background(), s, big.NewInt(20000), true, true, sqrtAtTick(-1000)); err == nil {
		t.Fatal("accepted missing initialized tick")
	}
	if _, err = c.quote(context.Background(), s, big.NewInt(20000), false, true, sqrtAtTick(1)); err == nil {
...
func TestQuoteDirectionsAndModes(t *testing.T) {
	for _, down := range []bool{true, false} {
		for _, exact := range []bool{true, false} {
			...
			r, err := c.quote(context.Background(), state, big.NewInt(1000), down, exact, limit)
			if err != nil {
				t.Fatal(err)
			}
			...
			if r.AmountIn.Uint64() != in || r.AmountOut.Uint64() != out || r.FeeAmount != fee {
...
// TestTurbosFeeRounding guards the deployed nearest-integer fee rule against a ceiling regression.
//
// Version:
//   - 2026-09-08: Added.
func TestTurbosFeeRounding(t *testing.T) {
...
// TestTurbosTokenBRounding verifies nearest rounding rather than a ceiling.
//
// Version:
//   - 2026-09-08: Added.
func TestTurbosTokenBRounding(t *testing.T) {
	q := new(big.Int).Lsh(big.NewInt(1), 64)
	b := new(big.Int).Add(q, big.NewInt(1))
	l := new(big.Int).Quo(new(big.Int).Mul(q, big.NewInt(6)), big.NewInt(5))
	if got := localDelta(q, b, l, false, true); got.Uint64() != 1 {
		t.Fatalf("delta %s", got)
	}
}

// TestProtocolShareAndZeroLiquidity verifies the separately reported share and incomplete fills.
//
// Version:
//   - 2026-09-08: Added.
func TestProtocolShareAndZeroLiquidity(t *testing.T) {
	c, f, p := stateFixture(t)
	f.obj.Move.JSON = json.RawMessage(strings.Replace(string(f.obj.Move.JSON), `"fee_protocol":100000`, `"fee_protocol":500000`, 1))
	r, e := c.QuotePair(context.Background(), p)
	if e != nil {
		t.Fatal(e)
	}
	if r.Bid.ProtocolFee != 1 || r.Ask.ProtocolFee != 1 {
		t.Fatalf("protocol shares %+v", r)
	}
	c.state = nil
	f.obj.Move.JSON = json.RawMessage(strings.Replace(string(f.obj.Move.JSON), `"liquidity":"1000000"`, `"liquidity":"0"`, 1))
	if _, e = c.QuotePair(context.Background(), p); e == nil {
		t.Fatal("accepted zero liquidity")
	}
}
