...
func (s *QuoteSnapshot) QuoteGrossPair(ctx context.Context, params QuotePairParams) (quotestate.GrossPair, error) {
	...
	return quotestate.GrossPair{BidAmountOut: new(big.Int).Set(gross.Bid.AmountOut), AskAmountIn: new(big.Int).Set(gross.Ask.AmountIn), BidFeeAmount: new(big.Int).SetUint64(net.Bid.FeeAmount), AskFeeAmount: new(big.Int).SetUint64(net.Ask.FeeAmount)}, nil
}
...
func (s *QuoteSnapshot) QuoteGrossExactInput(ctx context.Context, p QuoteExactInputParams) (quotestate.GrossExactInput, error) {
	...
	net, err := frozen.cache.quote(ctx, cloneRetainedState(frozen.cache.retained), p.AmountIn, p.A2B, true, p.SqrtPriceLimit)
	if err != nil {
		return quotestate.GrossExactInput{}, fmt.Errorf("failed to quote gross exact input: %w", err)
	}
	frozen.cache.retained.pool.Fee = 0
	gross, err := frozen.cache.quote(ctx, cloneRetainedState(frozen.cache.retained), p.AmountIn, p.A2B, true, p.SqrtPriceLimit)
	if err != nil {
		return quotestate.GrossExactInput{}, fmt.Errorf("failed to quote gross exact input: %w", err)
	}
	if err := ctx.Err(); err != nil {
	...
	return quotestate.GrossExactInput{AmountOut: new(big.Int).Set(gross.AmountOut), FeeAmount: new(big.Int).SetUint64(net.FeeAmount)}, nil
}
