...
func (c *StateCache) captureWindow(ctx context.Context, s *quoteState) error {
	words, nets, err := suiwindow.Read(ctx, c.reader, s.checkpoint, s.bitmap, s.ticks, s.keyType, s.pool.TickIndex, int32(s.pool.TickSpacing), jsonUnsigned)
	if err != nil {
		return fmt.Errorf("failed to capture momentum tick window: %w", err)
	}
...
func (c *StateCache) retainedCoverageMissing(ctx context.Context) bool {
	...
	missing := suiwindow.Missing(s.pool.TickIndex, int32(s.pool.TickSpacing), s.words, s.nets)
	c.retainedMu.Unlock()
	if missing {
		return true
	}
...
func (c *StateCache) captureRetainedWindow(ctx context.Context) (*StateCache, error) {
	...
	if c.retained == nil || c.retainedReference == nil {
		c.retainedMu.Unlock()
		return nil, fmt.Errorf("failed to capture momentum retained window: state=null")
	}
...
func (c *StateCache) installRetainedWindow(candidate *StateCache, updates []suiwindow.Update) error {
	if candidate == nil || candidate.retained == nil {
		return fmt.Errorf("failed to install momentum retained window: snapshot=null")
	}
	for _, update := range updates {
		if err := candidate.applyRetainedObjects(update.Notification, update.ReceivedAt); err != nil {
			return fmt.Errorf("failed to replay momentum retained window: %w", err)
		}
	}
	c.retainedMu.Lock()
	...
	if old == nil || next == nil {
		return fmt.Errorf("failed to install momentum retained window: state=null")
	}
	if next.version < old.version || candidate.retainedHead.SequenceNumber < c.retainedHead.SequenceNumber {
		return fmt.Errorf("failed to install momentum retained window: position=regressed")
	}
	if next.version == old.version && next.digest != old.digest {
		return fmt.Errorf("failed to install momentum retained window: digest=mismatch")
	}
