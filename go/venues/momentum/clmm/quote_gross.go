...
func (s *QuoteSnapshot) QuoteGrossExactInput(ctx context.Context, p QuoteExactInputParams) (quotestate.GrossExactInput, error) {
	...
	net, err := frozen.cache.quote(ctx, cloneRetainedState(frozen.cache.retained), p.AmountIn, p.XForY, true, p.SqrtPriceLimit)
	if err != nil {
		return quotestate.GrossExactInput{}, fmt.Errorf("failed to quote gross exact input: %w", err)
	}
	frozen.cache.retained.pool.FeeRate = 0
	gross, err := frozen.cache.quote(ctx, cloneRetainedState(frozen.cache.retained), p.AmountIn, p.XForY, true, p.SqrtPriceLimit)
	if err != nil {
		return quotestate.GrossExactInput{}, fmt.Errorf("failed to quote gross exact input: %w", err)
	}
	if err := ctx.Err(); err != nil {
