...
func localStep(price, target, l, remaining *big.Int, feeRate uint64, a2b, exactInput bool) (in, out, next, fee *big.Int, err error) {
	...
	if exactInput {
		...
		if !in.IsUint64() {
			err = fmt.Errorf("failed to calculate momentum step: input=out_of_range")
			return
		}
		...
		if !out.IsUint64() {
			err = fmt.Errorf("failed to calculate momentum step: output=out_of_range")
			return
		}
...
func localNext(price, l, amount *big.Int, a2b, exactInput bool) (*big.Int, error) {
	...
	if a2b == exactInput {
		...
		if d.Sign() <= 0 {
			return nil, fmt.Errorf("failed to calculate momentum next price: denominator=out_of_range")
		}
	...
	if next.Cmp(min) < 0 || next.Cmp(max) > 0 {
		return nil, fmt.Errorf("failed to calculate momentum next price: price=out_of_range")
	}
	return next, nil
}

// Momentum uses nearest-integer rounding here, unlike Cetus's ceiling.
