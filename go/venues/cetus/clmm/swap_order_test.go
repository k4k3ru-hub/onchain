...
func TestSwapTransactionOrder(t *testing.T) {
	for _, index := range []uint64{0, 3} {
		event := sui.LiveEvent{Checkpoint: 42, TransactionIndex: index, EventIndex: 7, Type: "0x1::pool::SwapEvent", JSON: json.RawMessage(`{"pool":"0x9","atob":true,"amount_in":"100","amount_out":"99","fee_amount":"1","before_sqrt_price":"10","after_sqrt_price":"11"}`)}
	...
	event := sui.Event{Checkpoint: 42, SequenceNumber: 7, Type: "0x1::pool::SwapEvent", JSON: json.RawMessage(`{"pool":"0x9","atob":true,"amount_in":"100","amount_out":"99","fee_amount":"1","before_sqrt_price":"10","after_sqrt_price":"11"}`)}
