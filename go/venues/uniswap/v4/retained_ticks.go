	...
	for start := progress.next; start < len(ticks); start += retainedTickBatchSize {
		...
		if batch, ok := c.rpc.(batchStateReader); ok {
			...
			for i, tick := range chunk {
				...
				calls[i] = append(append([]byte(nil), crypto.Keccak256([]byte("getTickLiquidity(bytes32,int24)"))[:4]...), append(c.poolID[:], arg.FillBytes(make([]byte, 32))...)...)
			}
			values, failures, err := batch.ReadContracts(ctx, c.stateView, calls, s.header.Number)
			if err != nil {
