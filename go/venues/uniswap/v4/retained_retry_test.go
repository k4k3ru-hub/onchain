...
func (r *retryWindowRPC) ReadContracts(ctx context.Context, target common.Address, calls [][]byte, block uint64) ([][]byte, []error, error) {
	...
	if string(calls[0][:4]) == string(crypto.Keccak256([]byte("getTickBitmap(bytes32,int16)"))[:4]) {
		r.bitmapCalls++
		fixture := tickWindowFixture()
		for i, call := range calls {
			word := int32(new(big.Int).SetBytes(call[36:]).Int64())
	...
	var ticks []int64
	for i, call := range calls {
		ticks = append(ticks, new(big.Int).SetBytes(call[36:]).Int64())
