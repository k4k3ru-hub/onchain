...
// Q96 arithmetic follows the integer rounding specified by Uniswap v4-core
...
func nextPrice(p, l, a *big.Int, zero, input bool) (*big.Int, error) {
	if l.Sign() == 0 {
		return nil, fmt.Errorf("failed to calculate uniswap v4 next price: liquidity=empty")
	}
	if zero == input {
		...
		if d.Sign() <= 0 {
			return nil, fmt.Errorf("failed to calculate uniswap v4 next price: denominator=out_of_range")
		}
	...
	if result.Sign() <= 0 {
		return nil, fmt.Errorf("failed to calculate uniswap v4 next price: sqrt_price=out_of_range")
	}
	return result, nil
}
...
func calculateStep(p, target, l, remaining *big.Int, fee uint32, input, zero bool) (localStep, error) {
	...
	var charged *big.Int
	if input && available.Cmp(needed) < 0 {
