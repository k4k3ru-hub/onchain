...
func (f *stateFake) CallContract(_ context.Context, msg ethereum.CallMsg, block *big.Int) ([]byte, error) {
	...
	switch sig {
	case selector("getSlot0(bytes32)"):
		args := abi.Arguments{{Type: mustABIType(f.t, "uint160")}, {Type: mustABIType(f.t, "int24")}, {Type: mustABIType(f.t, "uint24")}, {Type: mustABIType(f.t, "uint24")}}
		return args.Pack(power2(96), big.NewInt(0), big.NewInt(int64(f.protocol)), big.NewInt(3000))
	case selector("getLiquidity(bytes32)"):
	...
	case selector("getTickBitmap(bytes32,int16)"):
		n := new(big.Int)
		if f.crossed {
			if msg.Data[36] == 255 {
				n.SetBit(n, 255, 1)
	...
	case selector("getTickLiquidity(bytes32,int24)"):
		data := make([]byte, 64)
...
func newTestCache(t *testing.T) (*StateCache, *stateFake) {
	t.Helper()
	f := &stateFake{t: t}
	c, err := NewStateCache(f, PoolConfig{PoolManager: common.HexToAddress("01"), StateView: common.HexToAddress("02"), Currency1: common.HexToAddress("03"), Fee: 3000, TickSpacing: 60}, time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	return c, f
...
func TestStateSubscriptionInvalidatesAllLogsAndDisconnect(t *testing.T) {
	...
	ws.logs <- types.Log{Address: c.pool, Topics: []common.Hash{{}, c.poolID}, BlockNumber: 101, Removed: true}
	for {
		c.mu.Lock()
		floor := c.floor
		c.mu.Unlock()
...
func TestCoveredLogDoesNotInvalidateButRemovedLogDoes(t *testing.T) {
	...
	h := c.covered
	c.mu.Unlock()
	ws.logs <- types.Log{Address: c.pool, Topics: []common.Hash{{}, c.poolID}, BlockNumber: h.Number, BlockHash: h.Hash}
	ws.logs <- types.Log{Address: c.pool, Topics: []common.Hash{{}, c.poolID}, BlockNumber: h.Number, BlockHash: h.Hash, Removed: true}
	for {
		c.mu.Lock()
...
func TestObservedHashTracksLogsAndClearsOnRemovalAndDisconnect(t *testing.T) {
	...
	for _, log := range []types.Log{
		...
		c.mu.Unlock()
		log.Address = c.pool
		log.Topics = []common.Hash{{}, c.poolID}
		ws.logs <- log
...
func mustABIType(t *testing.T, name string) abi.Type {
	t.Helper()
	a, err := abi.NewType(name, "", nil)
	if err != nil {
		t.Fatal(err)
	}
	return a
}
func TestProtocolFeesAndUnsupportedPools(t *testing.T) {
	c, f := newTestCache(t)
	f.protocol = 1000 | (500 << 12)
	budget := 64
	s, err := c.capture(context.Background(), &budget, 0, common.Hash{})
	if err != nil {
		t.Fatal(err)
	}
	if s.fees != [2]uint32{3997, 3499} {
		t.Fatalf("fees=%v", s.fees)
	}
	f.protocol = 1001
	if _, err = c.capture(context.Background(), &budget, 0, common.Hash{}); err == nil {
		t.Fatal("invalid protocol fee accepted")
	}
	for _, cfg := range []PoolConfig{
		{PoolManager: c.pool, StateView: c.stateView, Currency1: common.HexToAddress("03"), Fee: 0x800000, TickSpacing: 60},
		{PoolManager: c.pool, StateView: c.stateView, Currency1: common.HexToAddress("03"), Fee: 3000, TickSpacing: 60, Hooks: common.HexToAddress("80")},
	} {
		if _, err := NewStateCache(f, cfg, time.Minute); err == nil {
			t.Fatal("unsupported pool accepted")
		}
	}
}

func TestPoolFilterAndReadBudget(t *testing.T) {
	c, f := newTestCache(t)
	if c.poolID == (common.Hash{}) || c.rpc != f || c.spacing != 60 || c.stateView != common.HexToAddress("02") {
		t.Fatal("dependencies not composed")
	}
	_, err := c.QuotePair(context.Background(), new(big.Int).Sub(power2(128), big.NewInt(1)), true)
	if err == nil || f.calls > 64 || c.snapshot != nil {
		t.Fatalf("unbounded or partial quote: calls=%d err=%v", f.calls, err)
	}
}

func TestStateSubscriptionIgnoresOtherPools(t *testing.T) {
	c, _ := newTestCache(t)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	ws := &stateWS{ready: make(chan struct{})}
	...
	go func() { done <- c.Run(ctx, ws) }()
	<-ws.ready
	ws.logs <- types.Log{Address: c.pool, Topics: []common.Hash{{}, common.HexToHash("dead")}, BlockNumber: 500}
	ws.logs <- types.Log{Address: c.pool, Topics: []common.Hash{{}, c.poolID}, BlockNumber: 101, BlockHash: common.HexToHash("01")}
	deadline := time.Now().Add(time.Second)
	for {
		c.mu.Lock()
		floor, gen := c.floor, c.generation
		c.mu.Unlock()
		if floor == 101 {
			if gen != 2 {
				t.Fatalf("other pool invalidated state: generation=%d", gen)
			}
			break
		}
		if time.Now().After(deadline) {
			t.Fatal("matching event not processed")
		}
		time.Sleep(time.Millisecond)
	}
	cancel()
