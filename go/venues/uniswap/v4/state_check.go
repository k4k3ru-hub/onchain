...
func (c *StateCache) CheckState(ctx context.Context) (result quotestate.Check, err error) {
	if c == nil || ctx == nil {
		return result, fmt.Errorf("failed to check uniswap v4 state: dependency=null")
	}
	select {
	case <-ctx.Done():
	...
	if old != nil {
		if fresh.header.Number < old.header.Number {
			return result, fmt.Errorf("failed to check uniswap v4 state: block=regressed")
		}
		for index := range old.words {
			data, readErr := c.read(ctx, fresh, "getTickBitmap(bytes32,int16)", big.NewInt(int64(index)), &budget)
			if readErr != nil {
				return result, fmt.Errorf("failed to read quote state: %w", readErr)
			}
			if len(data) != 32 {
				return result, fmt.Errorf("failed to check uniswap v4 state: bitmap=invalid")
			}
	...
	if err != nil {
		return result, fmt.Errorf("failed to verify uniswap v4 state: %w", err)
	}
	if header.Hash != fresh.header.Hash || header.Number != fresh.header.Number {
		return result, fmt.Errorf("failed to check uniswap v4 state: block_hash=mismatch")
	}
	...
	}{Epoch: c.verificationEpoch, Price: fresh.price, Liquidity: fresh.liquidity, Tick: fresh.tick, Spacing: fresh.spacing, Fees: fresh.fees, Words: fresh.words, Ticks: fresh.ticks})
	if err != nil {
		return result, fmt.Errorf("failed to encode uniswap v4 state: %w", err)
	}
	c.snapshot = fresh
