...
// TestRetainedMissingCoverageRequestsCapture verifies local failure signals without RPC.
//
// Version:
//   - 2026-09-10: Added.
func TestRetainedMissingCoverageRequestsCapture(t *testing.T) {
	c, rpc := newTestCache(t)
	amount := big.NewInt(1000000)
	s, err := c.captureRetainedWindow(context.Background(), amount, true, 0, common.Hash{})
	if err != nil {
		t.Fatal(err)
	}
	c.retained, c.running = s, true
	c.retainedBaseAmount = amount
	c.retainedRecovery = make(chan struct{}, 1)
	delete(s.words, 0)
	calls := rpc.calls
	for i := 0; i < 2; i++ {
		if _, err := c.QuoteRetainedPair(context.Background(), amount, true); !errors.Is(err, errStateReadBudget) {
			t.Fatalf("missing bitmap error = %v", err)
		}
	}
	if rpc.calls != calls || len(c.retainedRecovery) != 1 {
		t.Fatal("quote performed RPC or recovery requests were not coalesced")
	}
	<-c.retainedRecovery
	if _, err := c.QuoteRetainedPair(context.Background(), new(big.Int).Mul(amount, big.NewInt(2)), true); err == nil {
		t.Fatal("uncovered large quote succeeded")
	}
	if len(c.retainedRecovery) != 0 || rpc.calls != calls {
		t.Fatal("large quote triggered reference-sized capture")
	}
}
...
func TestRetainedThreeWordWindow(t *testing.T) {
	...
	if rpc.calls != 5 { // slot0, liquidity, and three bitmap reads (spacing is in PoolKey).
		t.Fatalf("contract calls = %d, want 5", rpc.calls)
	}
	c.retained = s
...
func TestRetainedWindowJumpFetchesNewNeighborhood(t *testing.T) {
	...
	if len(s.words) != 3 || s.words[6] == nil || s.words[7] == nil || s.words[8] == nil || fake.calls != 5 {
...
func (r *windowRPC) CallContract(ctx context.Context, msg ethereum.CallMsg, block *big.Int) ([]byte, error) {
	...
	if string(msg.Data[:4]) == string(crypto.Keccak256([]byte("getSlot0(bytes32)"))[:4]) {
...
func TestRetainedWindowRefreshKeepsStreamAndReplaysDeltas(t *testing.T) {
	...
	// An unrelated pool's removal must not terminate this subscription.
	ws.logs <- types.Log{Address: c.pool, Removed: true, Topics: []common.Hash{{}, common.HexToHash("abcdef")}}
	ws.logs <- mint
	feeLog := types.Log{Address: c.pool, BlockNumber: 102, BlockHash: windowHeader(102).Hash, Index: mint.Index + 1, Topics: []common.Hash{crypto.Keccak256Hash([]byte("ProtocolFeeUpdated(bytes32,uint24)")), c.poolID}, Data: big.NewInt(500 | 1000<<12).FillBytes(make([]byte, 32))}
	ws.logs <- feeLog
	waitState(func() bool { return c.retained != nil && c.retainedBlock == 102 && c.retainedIndex == feeLog.Index })
	close(release)
	waitState(func() bool { return c.retained != nil && c.retained.header.Number == 101 })
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.retained.fees != [2]uint32{3499, 3997} || c.retained.words[2] == nil || c.retainedBlock != 102 || c.retained.liquidity.Cmp(integer("1000000000000000100")) != 0 {
