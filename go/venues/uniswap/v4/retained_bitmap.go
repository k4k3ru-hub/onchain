	...
	if batch, ok := c.rpc.(batchStateReader); ok {
		...
		*budget -= count // Budget logical reads, not HTTP round trips.
		...
		for i := range calls {
			...
			calls[i] = append(append([]byte(nil), crypto.Keccak256([]byte("getTickBitmap(bytes32,int16)"))[:4]...), append(c.poolID[:], word.FillBytes(make([]byte, 32))...)...)
		}
		results, failures, err := batch.ReadContracts(ctx, c.stateView, calls, s.header.Number)
		if err != nil {
		...
		for i := range values {
			...
			value, err := c.read(ctx, s, "getTickBitmap(bytes32,int16)", big.NewInt(word), budget)
			if err != nil {
