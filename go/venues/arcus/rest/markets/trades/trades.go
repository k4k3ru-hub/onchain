// Package trades implements the Arcus Trades operation.
package trades
...
func (c *Client) Send(ctx context.Context, p Params) (*Result, error) {
	if c == nil {
		return nil, fmt.Errorf("failed to request trades: client=null")
	}
	if ctx == nil {
		return nil, fmt.Errorf("failed to request trades: context=null")
	}
	q, err := query.History(p.Market, p.Limit, p.From, p.To)
	if err != nil {
		return nil, fmt.Errorf("failed to request trades: %w", err)
	}

	var result Result
	if err := c.executor.Get(ctx, "/v1/trades", q, &result); err != nil {
		return nil, fmt.Errorf("failed to request trades: %w", err)
	}
	return &result, nil
}
