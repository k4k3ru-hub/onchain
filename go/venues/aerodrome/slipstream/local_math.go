...
func nextPrice(p, l, a *big.Int, zero, input bool) (*big.Int, error) {
	if l.Sign() == 0 {
		return nil, fmt.Errorf("failed to calculate slipstream next price: liquidity=empty")
	}
	if zero == input {
		...
		if d.Sign() <= 0 {
			return nil, fmt.Errorf("failed to calculate slipstream next price: denominator=out_of_range")
		}
	...
	if result.Sign() <= 0 {
		return nil, fmt.Errorf("failed to calculate slipstream next price: sqrt_price=out_of_range")
	}
	return result, nil
}
