...
func TestQuoteInputsAreDetached(t *testing.T) {
	c := retainedTestCache()
	c.retained.received = time.Unix(100, 0)
	c.retained.block, c.retained.hash, c.retained.index, c.retained.hasLog = 101, common.HexToHash("02"), 0, true
	c.retained.pool.words = map[int32]*big.Int{-2: big.NewInt(0), -1: big.NewInt(0), 2: big.NewInt(0)}
