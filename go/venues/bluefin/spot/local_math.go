...
func localStep(price, target, l, remaining *big.Int, feeRate uint64, a2b, exactInput bool) (in, out, next, fee *big.Int, err error) {
	...
	if exactInput {
		...
		if in.BitLen() > 256 {
			err = fmt.Errorf("failed to calculate bluefin step: input=out_of_range")
			return
		}
		...
		if out.BitLen() > 256 {
			err = fmt.Errorf("failed to calculate bluefin step: output=out_of_range")
			return
		}
...
func localNext(price, l, amount *big.Int, a2b, exactInput bool) (*big.Int, error) {
	...
	if a2b == exactInput {
		...
		if d.Sign() <= 0 {
			return nil, fmt.Errorf("failed to calculate bluefin next price: denominator=out_of_range")
		}
	...
	if next.Cmp(min) < 0 || next.Cmp(max) > 0 {
		return nil, fmt.Errorf("failed to calculate bluefin next price: price=out_of_range")
	}
	return next, nil
}

// Bluefin charges a ceiling fee when the target or exact output is reached.
func localFee(input *big.Int, rate uint64) *big.Int {
	return localDiv(new(big.Int).Mul(input, new(big.Int).SetUint64(rate)), new(big.Int).SetUint64(1000000-rate), true)
}
