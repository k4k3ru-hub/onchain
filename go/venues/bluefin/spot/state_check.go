...
func (c *StateCache) CheckState(ctx context.Context) (result quotestate.Check, err error) {
	if c == nil || ctx == nil {
		return result, fmt.Errorf("failed to check bluefin state: dependency=null")
	}
	select {
	case <-ctx.Done():
	...
	if head.Timestamp.IsZero() || time.Since(head.Timestamp) > c.maxAge || head.Timestamp.After(time.Now()) {
		return result, fmt.Errorf("failed to check bluefin state: checkpoint=invalid")
	}
	...
	if head.SequenceNumber < c.accepted.SequenceNumber || head.Timestamp.Before(c.accepted.Timestamp) {
		return result, fmt.Errorf("failed to check bluefin state: checkpoint=regressed")
	}
	if head.SequenceNumber == c.accepted.SequenceNumber && head.Digest != c.accepted.Digest {
		return result, fmt.Errorf("failed to check bluefin state: checkpoint_digest=mismatch")
	}
	...
	if obj == nil || obj.Address != c.pool || obj.Version == 0 {
		return result, fmt.Errorf("failed to check bluefin state: object=invalid")
	}
	...
	if err != nil {
		return result, fmt.Errorf("failed to encode bluefin state: %w", err)
	}
	if err = ctx.Err(); err != nil {
		return result, fmt.Errorf("failed to check quote state: %w", err)
	}
	if head.SequenceNumber.Uint64() < c.floor.Load() {
		return result, fmt.Errorf("failed to check bluefin state: %w: snapshot=invalidated", quotestate.ErrStateChanged)
	}
