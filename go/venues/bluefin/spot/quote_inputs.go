...
func (s *QuoteSnapshot) Inputs() *quotestate.Inputs {
	...
	return &quotestate.Inputs{UnavailableReason: reason, Baseline: baseline, Position: position, ReceivedAt: s.ReceivedAt(), CoverageUnit: "bitmap_word", Coverage: coverage, Fields: map[string]string{
		"sqrt_price": quotestate.Integer(pool.SqrtPrice), "fraction_bits": "64", "liquidity": quotestate.Integer(pool.Liquidity), "tick": strconv.FormatInt(int64(p.tickIndex), 10), "tick_spacing": strconv.FormatUint(uint64(p.spacing), 10), "fee_ppm": strconv.FormatUint(uint64(pool.FeeRate), 10), "pool_version": strconv.FormatUint(p.version, 10), "pool_digest": p.digest.String(),
	}}
}
