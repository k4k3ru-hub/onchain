...
func (c *StateCache) Warm(ctx context.Context) error {
	if c == nil {
		return fmt.Errorf("failed to warm bluefin state: cache=null")
	}
	if ctx == nil {
	...
	case c.gate <- struct{}{}:
	case <-ctx.Done():
		return fmt.Errorf("failed to warm bluefin state: %w", ctx.Err())
	}
	defer func() { <-c.gate }()
	head, err := sui.QuoteCheckpoint(ctx, c.reader, sui.CheckpointSequenceNumber(c.floor.Load()))
	if err != nil {
		return fmt.Errorf("failed to warm bluefin state: %w", err)
	}
	if head.Timestamp.IsZero() || time.Since(head.Timestamp) > c.maxAge {
		return fmt.Errorf("failed to warm bluefin state: checkpoint=invalid")
	}
	...
	if head.SequenceNumber < c.accepted.SequenceNumber || head.Timestamp.Before(c.accepted.Timestamp) {
		return fmt.Errorf("failed to warm bluefin state: checkpoint=regressed")
	}
	if head.SequenceNumber == c.accepted.SequenceNumber && head.Digest != c.accepted.Digest {
		return fmt.Errorf("failed to warm bluefin state: checkpoint_digest=mismatch")
	}
	...
	if obj == nil || obj.Address != c.pool || obj.Version == 0 {
		return fmt.Errorf("failed to warm bluefin state: object=invalid")
	}
	...
	if head.SequenceNumber.Uint64() < c.floor.Load() || time.Since(head.Timestamp) > c.maxAge || time.Since(c.state.captured) > c.maxAge {
		return fmt.Errorf("failed to warm bluefin state: %w: snapshot=invalidated", quotestate.ErrStateChanged)
	}
	c.accepted = head
	c.retainedMu.Lock()
